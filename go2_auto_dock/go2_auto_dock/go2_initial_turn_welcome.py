...
class AprilTagNode(Node):
    ...
    def __init__(self):
        ...
        # --- Welcome mode flag ---
        self.welcome_mode_enabled = False

        # --- ID=1 detection counter ---
        self.id1_detect_count = 0
        self.turn_180_flag = 0.0
    ...
    def imu_callback(self, msg: Imu):
        ...
        # unwrap yaw to avoid ±π jumps
        dyaw = yaw - self.current_yaw
        if dyaw > math.pi:
            dyaw -= 2 * math.pi
        elif dyaw < -math.pi:
            dyaw += 2 * math.pi
    ...
    def welcome_callback(self, msg: String):
        """
        Callback for /om/welcome topic.
        Enables AprilTag detector when "welcome mode" is detected in the message.
        ...
        msg : std_msgs.msg.String
            The welcome message.
        """
        if "welcome mode" in msg.data:
            self.welcome_mode_enabled = True
            self.get_logger().info(f"Welcome mode detected: {msg.data}")
    ...
    def timer_callback(self):
        ...
        # --- START 180 TURN ---
        if self.turn_180_flag == 1.0 and not self.turning_180:
            ...
            self.get_logger().warn("Starting PID-controlled 180° turn")
        ...
        if self.turning_180:
            ...
            # clamp for safety
            yaw_cmd = max(min(yaw_cmd, 2.0), -2.0)

            # send command
            self.send_move(0.01, 0.0, yaw_cmd)
            ...
            if abs(yaw_error) < math.radians(8.0):
                ...
                self.turning_180 = False
                self.turn_180_flag = 0.0
                self.get_logger().info("180° turn completed (PID)")
    ...
    def image_callback(self, msg: Image):
        ...
        """
        # Only process if welcome mode is enabled
        if not self.welcome_mode_enabled:
            return
        ...
        if 1 in tag_dict:
            ...
            # --- Count ID=1 detections ---
            if not self.turn_180_sent:
                self.id1_detect_count += 1
                self.get_logger().info(
                    f"[ID1 COUNTER] Detected ID=1 {self.id1_detect_count}/5"
                )

                if self.id1_detect_count >= 5:
                    self.turn_180_flag = 1.0
                    self.turn_180_sent = True
                    self.get_logger().warn(
                        "ID=1 detected 5 times → requesting 180° turn"
                    )
