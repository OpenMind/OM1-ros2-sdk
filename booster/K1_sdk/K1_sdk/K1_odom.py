#!/usr/bin/env python3
"""
K1 Odometry WebSocket Bridge.

Connects to K1 via WebSocket, requests odometry data,
and publishes to ROS2 /odom topic.

Usage:
    ros2 run <package> K1_odom_bridge.py

    or directly:
    python3 K1_odom_bridge.py
"""
...
class K1OdomBridge(Node):
    """ROS2 node that bridges K1 WebSocket odometry to ROS2 /odom topic."""
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f"K1 Odom Bridge started - Host: {self.host}, ACCID: {self.accid}"
        )

    def connect(self):
        """Connect to K1 WebSocket."""
        try:
